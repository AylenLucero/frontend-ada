const bulbasaur = {
  ...
  const resistsMove = (pokemon, movimiento) => {   
    ...
    const {type: tipo, modifiers:{resistances}}  = pokemon;    
        for(resistance of resistances) {
            if(type === resistance) {
                return true
            }  
            // booleano = (type === resistance);
        }    
    return false;
  }
  // let movimiento = {
...
addMove(pikachu, movimiento);
  ...
  // Crear una función removeMove que tome como argumentos un pokémon y un movimiento, elimine dicho movimiento de su lista y devuelva el pokémon con el movimiento desagregado
