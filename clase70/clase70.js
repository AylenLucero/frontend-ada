...
*/
let user={
    fullname:"Aylen Lyonnet",
    email:"lyonnet@gmail.com",
    age: 23
}
...
// console.log(personas[3].nombre, personas[3].id);
...
//>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>EJERCICIO 10<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<< 
//10- Crear una función que devuelva el volumen de una caja cuando se le pase un objeto con las medidas de la misma, correspondientes al ancho, alto, y largo. El volumen se calcula multiplicando entre sí dichos valores.
...
// (banda.activa===true)
//   ?console.log(`${banda.nombre} esta activa desde el año ${banda.fundacion}`)
//   :console.log(`${banda.nombre} no esta activa`);
// }  
///// RESULTADO
...
let promedio=0;
const cantidadMiembros= banda1.members.length;
const miembros= banda1.members;
const cantidadDiscos= banda1.albums.length;
for(banda of banda1.albums) {
    cantidadTotalCanciones+=banda.songs.length
    promedio+=banda.duration
    promedioDuracion=promedio/cantidadTotalCanciones
}
///// RESULTADO
console.log(banda1.name + " se fundó en el año " + banda1.songs);
